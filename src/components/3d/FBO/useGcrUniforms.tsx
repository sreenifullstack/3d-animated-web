...
export const useGcrUniforms = () => {
  ...
  const updateColorUniforms = useCallback((key: string, value: string) => {
    uniforms.current[key].value = new THREE.Color(value);
  }, []);
  ...
  const [opts, updateControls] = useControls("GCR", () => {
    return {
      anim2: {
        value: 0,
        min: 0,
        ...
        step: 0.01,
        label: "active wave",
        onChange: (v) => updateFloatUniforms("anim2", v),
      },
      timeScale: {
        value: 0.02,
        min: 0,
      ...
      waves: {
        ...
        max: 100,
        step: 0.01,
        onChange: (v) => updateFloatUniforms("waves", v),
      },
      frequency: {
        value: 1,
        min: 0,
        max: 4,
        step: 0.001,
        onChange: (v) => updateFloatUniforms("frequency", v),
      },
      xWaveScale: {
        value: 0.01,
        min: 0,
        max: 0.5,
        step: 0.001,
        onChange: (v) => updateFloatUniforms("xWaveScale", v),
      },
      yWaveScale: {
        value: 0.01,
        min: 0,
        max: 0.5,
        step: 0.001,
        onChange: (v) => updateFloatUniforms("yWaveScale", v),
      },
      zWaveScale: {
        value: 0.01,
        min: 0,
        max: 0.5,
        step: 0.001,
        onChange: (v) => updateFloatUniforms("zWaveScale", v),
      },
      lines: {
        value: 0,
        min: 0,
        max: 1,
        step: 0.01,
        onChange: (v) => updateFloatUniforms("lines", v),
      },
      uHole: {
        value: 0,
        min: 0,
        max: 1,
        step: 0.01,
        onChange: (v) => updateFloatUniforms("uHole", v),
      },
      uLineOP: {
        value: 0,
        min: 0,
        max: 1,
        step: 0.01,
        onChange: (v) => updateFloatUniforms("uLineOP", v),
      },
    };
  });
  ...
  const updateUniforms = useCallback(
    (controls: Record<string, any>) => {
      ...
      if (Object.keys(updatedUniforms).length) {
        //  = { ...uniforms.current, ...updatedUniforms };
